...
async function createCounterpartyThread(role: "vendor" | "user", input: { subject?: string; body: string }) {
  const res = await apiFetch<MessageThreadDetail>(`/portal/${role}/messages/threads`, {
    method: "POST",
    credentials: "include",
    cache: "no-store",
    body: { subject: input.subject?.trim() || undefined, body: input.body.trim() },
  });
  return unwrap(res);
}

export async function listAdminMessageThreads(params?: { page?: number; pageSize?: number; unreadOnly?: boolean }) {
  return listThreads("admin", params);
}

export async function listVendorMessageThreads(params?: { page?: number; pageSize?: number; unreadOnly?: boolean }) {
  return listThreads("vendor", params);
}

export async function listUserMessageThreads(params?: { page?: number; pageSize?: number; unreadOnly?: boolean }) {
...
export async function createVendorMessageThread(input: { subject?: string; body: string }) {
  return createCounterpartyThread("vendor", input);
}

export async function createUserMessageThread(input: { subject?: string; body: string }) {

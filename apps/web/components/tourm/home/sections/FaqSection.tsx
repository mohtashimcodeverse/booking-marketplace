      ...
      <div className="mx-auto w-full max-w-6xl px-4 sm:px-6">
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-2">
          {faqs.slice(0, 8).map((f, idx) => (
            <motion.details
              key={f.q}
              className="group rounded-3xl border border-stone bg-white p-6 shadow-sm"
            ...
            >
              <summary className="flex cursor-pointer list-none items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <span className="grid h-8 w-8 place-items-center rounded-2xl bg-sand text-sm font-extrabold text-midnight">
                    Q
                  </span>
                  <span className="text-base font-extrabold text-midnight">
                    {f.q}
                  </span>
                </div>

                <span className="relative h-6 w-6 text-ink/60">

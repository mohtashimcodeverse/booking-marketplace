        ...
        {c.lines.map((l) => (
          <div key={l.label} className="flex flex-col gap-1">
            ...
            {l.href ? (
              <Link
                href={l.href}
                className="text-sm font-semibold text-midnight transition hover:text-[#16a6c8]"
              >
                {l.value}
              </Link>
            ) : (
              <p className="text-sm font-semibold text-midnight">{l.value}</p>
            )}
          </div>
        ))}
      </div>

      <div className="mt-5 h-1.5 w-12 rounded-full bg-[#16a6c8]/15" />
    </div>
  );
}
